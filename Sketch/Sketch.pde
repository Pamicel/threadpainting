...
void setup() {
  ...
  gf = new GreatFeather(3 * width / 4, 5, 100);
  int nPoints = gf.path.length;
  float[] angles = new float[nPoints];
  int[] radii = new int[nPoints];
  float[] speeds = new float[nPoints - 1];
  ...
  for (int i = 0; i < nPoints; i++) {
    ...
    angles[i] = new Vec2D(0,1).angleBetween(pointB.sub(pointA), true);
  ...
  gfTrail = ToxiColorTrailFromPositions(
    physics,
    gf.path,
    angles,
    speeds,
  ...
  layer1Vars.baseColor = new int[] { 255, 215, 0 };
  layer1Vars.omega = realScale * 1.2;
...
void draw() {
  ...
  if (!gfTrail.finished()) {
    ...
    layer1.rotate(PI);
    gfTrail.colorString.displayOneInTwo(
      layer1,
      layer1Vars.rgbK,
